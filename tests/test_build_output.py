"""Sanity checks on the generated Thucydides build output."""
...
class TestWorks(unittest.TestCase):
    ...
    def test_locations_follow_catalog_order(self):
        self.assertEqual(self.works[0]["location"], "01.Thuc")
...
class TestParagraphs(unittest.TestCase):
    ...
    def test_paragraphs_are_present(self):
        self.assertEqual(len(self.paragraphs), 3587)

    def test_first_paragraph_shape(self):
        first = self.paragraphs[0]
        self.assertEqual(first["canonical_id"], "Thuc.1.1.1")
        self.assertEqual(first["location"], "01.Thuc.001.0001.001")
...
class TestTokens(unittest.TestCase):
    ...
    def test_thucydides_vocabulary_present(self):
        for word in ('καὶ', 'ἀθηναῖοι', 'λακεδαιμόνιοι'):
            self.assertIn(word, self.tokens)
        self.assertGreater(len(self.tokens["ἀθηναῖοι"]), 300)
...
class TestPublishedMetadata(unittest.TestCase):
    def test_hapax_counts_and_instance_json(self):
        ...
        self.assertEqual(instance["id"], "thucydides")
        self.assertEqual(instance["language"], "Greek")
        self.assertEqual(instance["created"], "2026-05-15")
        self.assertEqual(instance["stats"]["texts"], 1)
        self.assertEqual(instance["stats"]["segments"], 3587)
        self.assertEqual(instance["stats"]["words"], 150392)
    ...
    def test_characters_hold_per_book_aggregates_not_names(self):
        ...
        self.assertEqual(len(chars), 8)
        self.assertTrue(all(ch["name"].startswith("Βιβλίον") for ch in chars))
